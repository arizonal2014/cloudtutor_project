"""Browserbase-backed computer implementation for Computer Use."""
...
class BrowserbaseComputer(Computer):
    """Runs browser actions in a remote Browserbase session via CDP."""
    ...
    def __init__(
        ...
        *,
        ...
        self._browserbase: Any | None = None
    ...
    def __enter__(self) -> "BrowserbaseComputer":
        ...
        if not browserbase_api_key or not browserbase_project_id:
            raise ComputerUseConfigurationError(
                "Missing Browserbase configuration. "
        ...
        try:
            ...
            raise ComputerUseDependencyError(
                "Missing Browserbase Python SDK. Install with: "
                "'.venv/bin/pip install browserbase'."
            ) from exc
        ...
        self._browserbase = browserbase.Browserbase(api_key=browserbase_api_key)
        ...
        self.session_id = str(getattr(self._session, "id", "") or "")
        if self.session_id:
            try:
                live_urls = self._browserbase.sessions.debug(self.session_id)
                self.debug_url = (
                    getattr(live_urls, "debugger_fullscreen_url", None)
                    or getattr(live_urls, "debugger_url", None)
                    or f"https://browserbase.com/sessions/{self.session_id}"
                )
            except Exception:
                self.debug_url = f"https://browserbase.com/sessions/{self.session_id}"
    ...
    def _handle_new_page(self, new_page: Any) -> None:
        """Normalizes behavior to a single active tab for CUA execution."""
